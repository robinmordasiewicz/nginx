...
#xvfb-run -n 99 -a --listen-tcp --server-args="-screen 0 1280x720x24 -ac -nolisten tcp -dpi 96 +extension RANDR" "./puppeteer.sh $1 $2" &
#xvfb-run -n 99 -a --listen-tcp --server-args="-screen 0 1280x720x24 -ac -nolisten tcp -dpi 96 +extension RANDR" sleep 10 ; node volterra.js $1 $2 &
...
ffmpeg -v quiet -stats -video_size 1280x720 -r 30 -y -f x11grab -draw_mouse 0 -i :99 -an -c:v libx264rgb -crf 0 -analyzeduration 100M -probesize 400M -tune zerolatency -preset ultrafast -qp 0 -b:v 500k -t 75 screenrecording.mkv
