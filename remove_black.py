...
def construct_ffmpeg_trim_cmd(timepairs, inpath, outpath):
  cmd = f'ffmpeg -i "screengrab.mp4" -y -filter_complex '
  cmd += '"'
  ...
  cmd += '"'
  cmd += f' -map [outv] -map [outa] "outputa.mp4"'
  return cmd


def get_blackdetect(inpath, invert=False):
  ffprobe_cmd = f"ffprobe -f lavfi -i \"movie=screengrab.mp4,blackdetect[out0]\" -show_entries tags=lavfi.black_start,lavfi.black_end -of default=nw=1 -v quiet"
